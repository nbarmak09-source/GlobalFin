...
// TODO: BottleneckMonitor is hardcoded to SUPPLY_CHAIN_LAYERS_DATA (semiconductor layers)
// and does not accept a data prop. Skip it here for Oil & Gas until BottleneckMonitor
// is made injectable. See src/components/dashboard/BottleneckMonitor.tsx.

/** Left border accent per Oil & Gas layer — do NOT reuse semiconductor L1–L6 keys. */
const LAYER_LEFT_BORDER: Record<string, string> = {
  "OG-L1": "#d97706", // amber  — upstream / wellhead
  "OG-L2": "#0369a1", // sky    — midstream / pipelines
  "OG-L3": "#0d9488", // teal — downstream / products
  "OG-L4": "#7c3aed", // violet — OFS / technology
};
...
function LayersView({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/data/oil-gas-layers.json");
        if (!res.ok) throw new Error("Failed to load Oil & Gas supply chain data");
        const json = (await res.json()) as SupplyChainLayersFile;
        if (!cancelled) setData(json);
      } catch {
        if (!cancelled) setLoadError("Could not load Oil & Gas supply chain layers.");
      }
    })();
    return () => {
      cancelled = true;
...
function CommodityFlowsView() {
  return (
    <div className="rounded-xl border border-border bg-card p-12 text-center text-muted">
      <TrendingUp className="h-8 w-8 mx-auto mb-3 opacity-30" />
      <p className="text-sm font-medium">Commodity Flows map coming soon.</p>
      <p className="text-xs mt-1 opacity-70">
        Global crude, gas, and LNG flow visualisation is under development.
      </p>
    </div>
  );
}

type OilGasSubView = "layers" | "commodity-flows";

function parseSubView(raw: string | null): OilGasSubView {
  if (raw === "commodity-flows") return "commodity-flows";
  return "layers";
}

export default function OilGasView() {
  ...
  function setSubView(v: OilGasSubView) {
    const params = new URLSearchParams();
    params.set("industry", "oil-gas");
  ...
  return (
    <div className="space-y-4 min-w-0">
      ...
      <div className="flex items-center gap-1 rounded-lg bg-card border border-border p-0.5 w-fit">
        ...
        <button
          type="button"
          onClick={() => setSubView("commodity-flows")}
          className={`flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-md transition-colors ${
            subView === "commodity-flows"
              ? "bg-accent text-white"
              : "text-muted hover:text-foreground"
          }`}
        >
      ...
      {subView === "commodity-flows" && <CommodityFlowsView />}
    </div>
  );
}
